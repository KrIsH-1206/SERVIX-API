...
public class ProviderRepository {
    ...
    public ProviderRepository(JdbcTemplate db) {
        this.db = db;
    }

    // ── Pending Jobs with Location ──────────────────────────
    public List<Map<String, Object>> getPendingJobs(int providerId, int pageSize, int rowOffset) {
        return db.queryForList("""
                ...
                LIMIT ? OFFSET ?
                """, providerId, pageSize, rowOffset);
    }

    // ── Completed Jobs ──────────────────────────────────────
    public List<Map<String, Object>> getCompletedJobs(int providerId, int pageSize, int rowOffset) {
        return db.queryForList("""
                SELECT b.booking_id, cu.name AS customer, b.scheduled_date,
